...
std::vector<BATCH1DDFTProblem> b1dproblems_1d;
std::vector<IBATCH1DDFTProblem> ib1dproblems_1d;
...
fftx_plan fftx_plan_distributed_1d(
  ...
  plan->Q3 = new double[M*e*N*e*K*e / p * sizeof(complex<double>) * batch];
  plan->Q4 = new double[M*e*N*e*K*e / p * sizeof(complex<double>) * batch];
  /*
    R2C is
    [K, N,       M]
    [K, N, M/2 + 1]
  ...
  */
  ...
  if (plan->is_complex) {
    ...
    // );
    std::vector<int> size = {inM, batch_sizeX, 0, 1};  
      BATCH1DDFTProblem bdstg1(size, "b1dft");
      b1dproblems_1d.push_back(bdstg1);
    ...
    // );
    std::vector<int> size2 = {inN, batch_sizeY, 0, 1};  
    BATCH1DDFTProblem bdstg2(size2, "b1dft");
    b1dproblems_1d.push_back(bdstg2);
    ...
    // );
    std::vector<int> size3 = {inK, batch_sizeZ, 0, 0};  
    BATCH1DDFTProblem bdstg3(size3, "b1dft");	
    b1dproblems_1d.push_back(bdstg3);
    ...
    //     DEVICE_FFT_Z2Z, batch_sizeY
    // );
    std::vector<int> size2_inv = {inN, batch_sizeY, 1, 0};  
    IBATCH1DDFTProblem ibdstg2(size2, "ib1dft");
    ib1dproblems_1d.push_back(ibdstg2);
    std::cout << "finished second stage problem" << std::endl;
    ...
    //     DEVICE_FFT_Z2Z, batch_sizeX
    // );
    std::vector<int> size_inv = {inM, batch_sizeX, 1, 0};  
  ...
  if (direction == DEVICE_FFT_FORWARD) {
    if (plan->is_complex) {
      ...
      for (int i = 0; i < plan->b; i++) {
        ...
        b1dproblems_1d.at(0).setArgs(args);
        b1dproblems_1d.at(0).transform();
      }
      ...
      for (int i = 0; i < plan->b; ++i) {
        ...
        b1dproblems_1d.at(1).setArgs(args);
        b1dproblems_1d.at(1).transform();
      }
      ...
      for (int i = 0; i < plan->b; ++i) {
        ...
        b1dproblems_1d.at(2).setArgs(args);
        b1dproblems_1d.at(2).transform();
      }
    } else {
      //forward real
      // [X', Z/p, Y, b] <= [Z/p, Y, X, b]
    ...
    for (int i = 0; i < plan->b; ++i) {
      ...
      ib1dproblems_1d.at(0).setArgs(args);
      ib1dproblems_1d.at(0).transform();
    }
    ...
    for (int i = 0; i < plan->b; ++i) {
      if (plan->is_complex) {
        ...
        ib1dproblems_1d.at(1).setArgs(args);
        ib1dproblems_1d.at(1).transform();
      } else {
        //backward real
