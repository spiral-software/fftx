  ...
  if ((plan->is_complex))
    {
      ...
      FFTX_DEVICE_FFT_PLAN_MANY(&(plan->stg1), 1, &inK,
			   &inK,             plan->b, inK*plan->b,
			   &inK, batch_sizeZ*plan->b, plan->b,
			   FFTX_DEVICE_FFT_Z2Z, batch_sizeZ);

      //inverse plan -> read strided write seq
      FFTX_DEVICE_FFT_PLAN_MANY(&(plan->stg1i), 1, &inK,
			   &inK, batch_sizeZ*plan->b, plan->b,
			   &inK,             plan->b, inK*plan->b,
			   FFTX_DEVICE_FFT_Z2Z, batch_sizeZ);
  ...
  else
    {
      ...
      FFTX_DEVICE_FFT_PLAN_MANY(&(plan->stg1), 1, &inK,
			   &inK,             plan->b, inK*plan->b,
			   &inK, batch_sizeZ*plan->b, plan->b,
			   FFTX_DEVICE_FFT_D2Z, batch_sizeZ);

      //inverse plan -> read strided write seq
      FFTX_DEVICE_FFT_PLAN_MANY(&(plan->stg1i), 1, &inK,
			   &inK, batch_sizeZ*plan->b, plan->b,
			   &inK,             plan->b, inK*plan->b,
			   FFTX_DEVICE_FFT_Z2D, batch_sizeZ);
    }

  //read seq write strided
  FFTX_DEVICE_FFT_PLAN_MANY(&(plan->stg2), 1, &inM,
		       &inM,           plan->b, inM*plan->b,
		       &inM, batch_sizeX*plan->b, plan->b,
		       FFTX_DEVICE_FFT_Z2Z, batch_sizeX);

  //read seq write seq
  FFTX_DEVICE_FFT_PLAN_MANY(&(plan->stg3), 1, &inN,
		       &inN, plan->b, inN*plan->b,
		       &inN, plan->b, inN*plan->b,
		       FFTX_DEVICE_FFT_Z2Z, batch_sizeY);

  //read strided write seq
  FFTX_DEVICE_FFT_PLAN_MANY(&(plan->stg2i), 1, &inM,
		       &inM, batch_sizeX*plan->b, plan->b,
		       &inM,           plan->b, inM*plan->b,
		       FFTX_DEVICE_FFT_Z2Z, batch_sizeX);
  ...
  if (direction == FFTX_DEVICE_FFT_FORWARD) {
    ...
    if (plan->is_complex) {
      for (int i = 0; i != plan->b; ++i) {
        FFTX_DEVICE_FFT_EXECZ2Z(plan->stg1, ((FFTX_DEVICE_FFT_DOUBLECOMPLEX  *) in_buffer + i), ((FFTX_DEVICE_FFT_DOUBLECOMPLEX  *) plan->Q3 + i), direction);
      }
    } else {
      for (int i = 0; i != plan->b; ++i) {
        FFTX_DEVICE_FFT_EXECD2Z(plan->stg1, ((FFTX_DEVICE_FFT_DOUBLEREAL  *) in_buffer + i), ((FFTX_DEVICE_FFT_DOUBLECOMPLEX  *) plan->Q3 + i));
      }
    }
    // stop = MPI_Wtime();
    ...
    // start = MPI_Wtime();
    for (int i = 0; i != plan->b; ++i) {
      FFTX_DEVICE_FFT_EXECZ2Z(plan->stg2, ((FFTX_DEVICE_FFT_DOUBLECOMPLEX  *) plan->Q4 + i), ((FFTX_DEVICE_FFT_DOUBLECOMPLEX  *) plan->Q3 + i), direction);
    }
    // stop = MPI_Wtime();
    // max_time = max_diff(start, stop, plan->all_comm);
    // if (rank == 0) { printf("%f,", max_time); }
    ...
    // start = MPI_Wtime();
    for (int i = 0; i != plan->b; ++i) {
      FFTX_DEVICE_FFT_EXECZ2Z(plan->stg3, ((FFTX_DEVICE_FFT_DOUBLECOMPLEX  *) plan->Q4 + i), ((FFTX_DEVICE_FFT_DOUBLECOMPLEX  *) out_buffer + i), direction);
    }
    // stop = MPI_Wtime();
    // max_time = max_diff(start, stop, plan->all_comm);
    // if (rank == 0) { printf("%f,", max_time); }
    ...
    for (int i = 0; i != plan->b; ++i) {
      FFTX_DEVICE_FFT_EXECZ2Z(
        plan->stg3,
        ((FFTX_DEVICE_FFT_DOUBLECOMPLEX  *) in_buffer + i),
        ((FFTX_DEVICE_FFT_DOUBLECOMPLEX  *) plan->Q3 + i),
        direction
      );
    }
    ...
    for (int i = 0; i != plan->b; ++i){
      FFTX_DEVICE_FFT_EXECZ2Z(plan->stg2i, ((FFTX_DEVICE_FFT_DOUBLECOMPLEX  *) plan->Q4 + i), ((FFTX_DEVICE_FFT_DOUBLECOMPLEX  *) plan->Q3 + i), direction);
    }
    ...
    if (plan->is_complex) {
      for (int i = 0; i != plan->b; ++i) {
        FFTX_DEVICE_FFT_EXECZ2Z(plan->stg1i, ((FFTX_DEVICE_FFT_DOUBLECOMPLEX  *) plan->Q4 + i), ((FFTX_DEVICE_FFT_DOUBLECOMPLEX  *) out_buffer + i), direction);
      }
    } else { // untested
      for (int i = 0; i != plan->b; ++i) {
        FFTX_DEVICE_FFT_EXECZ2D(plan->stg1i, ((FFTX_DEVICE_FFT_DOUBLECOMPLEX  *) plan->Q4 + i), ((FFTX_DEVICE_FFT_DOUBLEREAL  *) out_buffer + i));
      }
    }
  }
}
