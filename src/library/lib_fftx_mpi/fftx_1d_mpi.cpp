    ...
    case FFTX_MPI_EMBED_1:
      {
          ...
          size_t buffer_size = plan->shape[1] * plan->shape[0] * plan->shape[4] * plan->shape[2] * plan->b;
    ...
    case FFTX_MPI_EMBED_4:
      {
        ...
        if (is_embedded) {
          ...
          // printf("packing\n");
          // NOTE: assuming CPU packing, pack_embed uses send-recv buffers.
          DEVICE_MEM_COPY(
            plan->recv_buffer, X,
  ...
  if (direction == DEVICE_FFT_FORWARD) {
    ...
    } else
    {
      ...
      for (int i = 0; i < plan->b; i++) {
        // double tmp;
        // DEVICE_MEM_COPY(
        //   &tmp, ((DEVICE_FFT_DOUBLEREAL    *) in_buffer) + i,
        //   sizeof(double),
        //   MEM_COPY_DEVICE_TO_HOST
        // );
        // printf("%d %f\n", i, tmp);
    ...
    for (int i = 0; i < plan->b; ++i) {
      // double tmp;
      // DEVICE_MEM_COPY(
      //   &tmp, ((DEVICE_FFT_DOUBLECOMPLEX *) plan->Q4) + i,
      //   sizeof(double),
      //   MEM_COPY_DEVICE_TO_HOST
      // );
      // printf("%d %f\n", i, tmp);
    ...
    for (int i = 0; i < plan->b; ++i) {
      // double tmp;
      // DEVICE_MEM_COPY(
      //   &tmp, ((DEVICE_FFT_DOUBLECOMPLEX *) stg3_input) + i,
      //   sizeof(double),
      //   MEM_COPY_DEVICE_TO_HOST
      // );
      // printf("%d %f\n", i, tmp);
