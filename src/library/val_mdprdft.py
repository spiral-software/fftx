...
def setup_input_array ( dims, fwd ):
    ...
    if fwd:
        ...
        ##  Setup source (complex input) data -- shape = ( dx, dy, adjusted dz )
        array_shape = ( dims[0], dims[1], ( dims[2] // 2 + 1 ) )
        src = np.random.random ( array_shape ) + np.random.random ( array_shape ) * 1j

    return src;


def run_python_version ( src, dims, fwd ):
    "Create the output array by calling NumPy"

    if fwd:
        dst = np.fft.rfftn ( src )
        dst = dst / np.size ( dst )                     ##  Normalize results of forward xform
    else:
        dst = np.fft.irfftn ( src, tuple(dims) )        ##  Inverse transform is already normalized
...
def exec_xform ( libdir, libfwd, libinv, libext, dims, fwd, platform, typecode, warn ):
    ...
    src = setup_input_array ( dims, fwd )
    ...
    if fwd:
        ##  Destination (complex output) -- fill with zeros, adjusted Z dimension
        array_shape = ( dims[0], dims[1], ( dims[2] // 2 + 1 ) )
        _dst_spiral = np.zeros ( array_shape ).astype(complex)
    else:
        ##  Destination (real output) -- fill with zeros
        array_shape = tuple(dims)
    ...
    _dst_python = run_python_version ( src, dims, fwd )
