...
void Executor::getVars() {
    for(int i = 0; i < device_names.size(); i++) {
        NVRTC_SAFE_CALL(hiprtcAddNameExpression(prog, std::get<0>(device_names[i]).c_str()));
...
void Executor::getLogsAndPTX() {
    ...
    CUDA_SAFE_CALL(hipLinkCreate(0, 0, 0, &linkState));
    CUDA_SAFE_CALL(hipLinkAddFile(linkState, CU_JIT_INPUT_LIBRARY, getCUDARuntime(), 
    0, 0, 0));
    CUDA_SAFE_CALL(hipLinkAddData(linkState, CU_JIT_INPUT_PTX,
    (void *)ptx, ptxSize, "dft_jit.ptx",
    0, 0, 0));
    CUDA_SAFE_CALL(hipLinkComplete(linkState, &cubin, &cubinSize));
...
void Executor::initializeVars() {
    for(int i = 0; i < device_names.size(); i++) {
        ...
        // }
        const char * name;
        NVRTC_SAFE_CALL(hiprtcGetLoweredName(
        prog, 
        std::get<0>(device_names[i]).c_str(), // name expression
        &name                         // lowered name
        ));
        CUdeviceptr variable_addr;
        CUDA_SAFE_CALL(hipModuleGetGlobal(&variable_addr, NULL, module, name));
        std::string test = std::get<2>(device_names[i]);
        switch(hashit(test)) {
            case zero:
            {
                int * value = (int*)(data.at(i));
                CUDA_SAFE_CALL(hipMemcpyHtoD(variable_addr, value, std::get<1>(device_names.at(i))*sizeof(int)));
                break;
            }
            case one:
            {
                float * value = (float*)(data.at(i));
                CUDA_SAFE_CALL(hipMemcpyHtoD(variable_addr, value, std::get<1>(device_names.at(i))*sizeof(float)));
                break;
            }
            case two:
            {   
                double * value = (double*)(data.at(i));
                CUDA_SAFE_CALL(hipMemcpyHtoD(variable_addr, value, std::get<1>(device_names.at(i))*sizeof(double)));
                break;
            }
            case constant:
            {
...
float Executor::initAndLaunch(std::vector<void*>& args) {
    ...
    CUevent start, stop;
    CUDA_SAFE_CALL(hipEventCreate(&start, CU_EVENT_DEFAULT));
    CUDA_SAFE_CALL(hipEventCreate(&stop, CU_EVENT_DEFAULT));
    ...
    CUDA_SAFE_CALL(hipEventElapsedTime(&GPUtime, start, stop));
