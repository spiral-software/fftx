...
if 1 = 1 then
    ...
    ## name := "fftx_mddft"::StringInt(d)::"d_"::StringInt(szcube[1])::ApplyFunc(ConcatenationString, List(Drop(szcube, 1), s->"x"::StringInt(s)));
    ## Drop the dimensionality from source file name.
    name := "fftx_mddft"::"_"::szstring;
    
    PrintLine("fftx_mddft-cuda-frame: name = ", name, ", cube = ", szcube, ", size = ",
    ...
    t := TFCall(TDecl(TDAG([
        ...
        rec(fname:="fftx_mddft_"::szstring, params:= [symvar])
    );
    prefix:="fftx_mddft";
