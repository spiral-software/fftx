...
if 1 = 1 then
    ...
    ## name := "fftx_imddft"::StringInt(d)::"d_"::StringInt(szcube[1])::ApplyFunc(ConcatenationString, List(Drop(szcube, 1), s->"x"::StringInt(s)));
    ## Drop the dimensionality from source file name.
    name := "fftx_imddft"::"_"::szstring;
    
    PrintLine("fftx_imddft-hip-frame: name = ", name, ", cube = ", szcube, ", size = ",
              StringInt(szcube[1])::ApplyFunc(ConcatenationString, List(Drop(szcube, 1),
                                                                    s->" x "::StringInt(s))),
              ";\t\t##PICKME##");

    ## This line from imddft-frame-hip.g :
    ...
    t := TFCall(TDecl(TDAG([
           TDAGNode(TTensorI(MDDFT(szcube,1),1,APar, APar), var_3,var_2),
                  ]),
            [var_1]
            ),
        rec(fname:="fftx_imddft_"::szstring, params:= [symvar])
    );
    prefix:="fftx_imddft";
