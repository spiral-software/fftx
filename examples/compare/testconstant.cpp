                  ...
                  int a_verbosity)
{
  ...
  */
  ...
  double prodsize = a_tfmSize[0] * a_tfmSize[1] * a_tfmSize[2];
  double maxAbsDiff = abs(outputHostPtr[0] - prodsize);
  for (size_t ind = 1; ind < nptsOutput; ind++)
    {
      double absDiff = abs(outputHostPtr[ind]);
  ...
  if (a_verbosity >= 3)
    {
      std::cout << "Writing out anything nonzero in output" << std::endl;
               ...
               int a_verbosity)
{
  ...
  */
  ...
  double prodsize = tfmSize[0] * tfmSize[1] * tfmSize[2];
  double maxAbsDiff = abs(outputHostPtr[0] - prodsize);
  for (size_t ind = 1; ind < nptsOutput; ind++)
    {
      double absDiff = abs(outputHostPtr[ind]);
