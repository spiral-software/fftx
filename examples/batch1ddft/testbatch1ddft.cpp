...
int main(int argc, char* argv[])
{
    ...
    //APar = 0 AVec = 1
    if(read == 0 && write == 0) //TTensorI(DFT(N), batch, APar, APar)
    ...
    else if(read == 0 && write == 1)  //TTensorI(DFT(N), batch, AVec, APar)
        res = DEVICE_FFT_PLAN_MANY(&plan, 1, &N, //plan, rank, n
                                    &N, 1, N, // iembed, istride, idist,
                                    &N, B, 1, // oembed, ostride, odist,
                                    xfmtype, B); // type and batch
    else if(read == 1 && write == 0) //TTensorI(DFT(N), batch, APar, AVec)
    {
        std::cout << "AVEC, APAR" << std::endl;
        res = DEVICE_FFT_PLAN_MANY(&plan, 1, &N, 
                                    &N, B, 1, 
                                    &N, 1, N, 
                                    xfmtype, B);
    }
    else //TTensorI(DFT(N), batch, AVec, AVec)
        res = DEVICE_FFT_PLAN_MANY(&plan, 1, &N, &N, B, 1, &N, B, 1, xfmtype, B);
