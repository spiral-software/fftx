...
// static void checkOutputBuffers( DEVICE_FFT_DOUBLEREAL *spiral_Y, DEVICE_FFT_DOUBLEREAL *devfft_Y, long arrsz )
// {
//     bool correct = true;
//     double maxdelta = 0.0;
...
int main(int argc, char* argv[])
{
  ...
  while ( argc > 1 && argv[1][0] == '-' ) {
      ...
      case 's':
          ...
          break;
      case 'h':
          printf ( "Usage: %s: [ -i iterations ] [ -s <input_cube>x<padded_cube>x<output_cube> ] [ -h (print help message) ]\n", argv[0] );
          exit (0);
      default:
  ...
  DEVICE_MALLOC(&dY, outputHost.m_domain.size() * sizeof(double));

  DEVICE_MALLOC(&dsym,  symbolHost.m_domain.size() * sizeof(std::complex<double>));
...
for(int itn = 0; itn < iterations; itn++) 
{
  ...
  DEVICE_MEM_COPY(dsym, symbolHost.m_data.local(),  symbolHost.m_domain.size() * sizeof(std::complex<double>),
                  MEM_COPY_HOST_TO_DEVICE);
#endif
  if ( DEBUGOUT ) std::cout << "copied inputs\n";
